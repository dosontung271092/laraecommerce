...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
   <h1 class="h2">{{ __('labels.post-edit') }}</h1>
...
<form action="{{ url('admin/post/'.$post->id) }}" method="POST" enctype="multipart/form-data">
   ...
   <div class="row">
      ...
      <div class="col-md-6 mb-3">
         ...
         <input type="checkbox" name="status" {{ ( $post->status ) ? 'checked' : '' }}> Chọn=Ẩn, Bỏ chọn=Hiển thị
      </div>

      <div class="col-md-12 mb-3">
         <label>Nội dung</label>
         <textarea name="content" rows="5" class="form-control" id="editor">{{ $post->content }}</textarea>
         @error('content')
         <small class="text-danger">{{$message}}</small>
         @enderror
      </div>
      ...
      <div class="col-md-12 mb-3">
         ...
         <textarea type="text" name="meta_content" class="form-control">{{ $post->meta_content }}</textarea>
         @error('meta_content')
         <small class="text-danger">{{$message}}</small>
         @enderror
      </div>
